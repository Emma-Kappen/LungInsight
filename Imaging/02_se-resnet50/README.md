...
This repository now includes a 3D SE-ResNet50 model designed for volumetric medical imaging inputs.

- Input shape: `(batch, 1, 64, 64, 64)`
- Output: dictionary of 9 independent sigmoid heads
- Head names: `subtlety`, `internalStructure`, `calcification`, `sphericity`, `margin`, `lobulation`, `spiculation`, `texture`, `malignancy`
...
print(outputs['malignancy'].shape)  # torch.Size([4])
```

This is useful for integrating volumetric patches extracted by `lidc-preprocessing` into an explainable multi-head classification pipeline.
