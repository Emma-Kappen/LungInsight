...
from senet.se_module import SELayer, SELayer3d
...
class SEResNet3d(nn.Module):
    def __init__(self, block, layers, num_heads=len(FEATURE_NAMES_3D), in_channels=1,
                 reduction=16, head_names=None):
        ...
        self.head_names = head_names or FEATURE_NAMES_3D
        self.heads = nn.ModuleDict({
            name: nn.Linear(512 * block.expansion, 1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, reduction=16):
        ...
        layers = [block(self.inplanes, planes, stride, downsample, reduction=reduction)]
...
def se_resnet50_3d(in_channels=1, reduction=16, head_names=None):
    model = SEResNet3d(
        Bottleneck3d,
        [3, 4, 6, 3],
